...
if (!is_array($mediaSources)) { 
    $modx->log(modX::LOG_LEVEL_FATAL,'Adding mediaSources failed.'); 
} 
else{
...
unset($mediaSources, $mediaSource);


/* Load transport provider */
$vehicleParams = array(
    xPDOTransport::PRESERVE_KEYS => false,
    xPDOTransport::UPDATE_OBJECT => false,
    xPDOTransport::UNIQUE_KEY => 'service_url',
);

$providers = include $sources['data'].'transport.transport_providers.php';
if (!is_array($providers)) { 
    $modx->log(modX::LOG_LEVEL_FATAL,'Adding providers failed.'); 
} 
else{
    foreach($providers as & $provider){
        $vehicle = $builder->createVehicle($provider, $vehicleParams);
        $builder->putVehicle($vehicle);
    }
    $modx->log(modX::LOG_LEVEL_INFO,'Packaged in '.count($providers).' providers.');
}
